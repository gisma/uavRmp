---
title: "uavRmp - Unmanned aerial Vehicle Remote Sensing Tools "
...
This version of the document was produced on _`r format(Sys.time(), "%a %b %d %Y")`_ using `uavRmp` version **`r packageVersion("uavRmp")`**

------

# Installation
The `uavRmp` package needs the `library("devtools")`:

```{r, eval = FALSE, warning=FALSE, message=FALSE, fig.width=9.47}
devtools::install_github("gisma/uavRmp")
```
