if (!isGeneric('ggeodata')) {
  setGeneric('ggeodata', function(x, ...)
    standardGeneric('ggeodata'))
}
#'@name ggeodata
#'
#'@title Retrieves online geodata and converts it to raster/sp objects
#'
#'@description Robert J. Hijmans getData() from the raster package is well known and highly used. The only disadvantage is that it currently doesn't support a bunch of great additional and/or improved/newer data sets.  ggeodata provides some more actual or better choices for climate and DEM data as well as some easy to use interfaces to OSM and other crowd sourced data compilations.
...
#'
#'@usage ggeodata(name, download=TRUE, path='', ...)
#' ccodes()
#'
...
#'     \code{ggeodata('GADM', country='FRA', level=1)}\cr
#'     \code{ggeodata('alt', country='FRA', mask=TRUE)}\cr
#'     }
#'     
#'\subsection{SRTM}{
...
#'For example:\cr
#'   \code{ggeodata('CMIP5', var='tmin', res=10, rcp=85, model='AC', year=70)}\cr
...
#'    \code{ggeodata('worldclim', var='tmin', res=0.5, lon=5, lat=45)} \cr
#'    \code{ggeodata('worldclim', var='bio', res=10)}\cr\cr
#'}
#'
...
#'}
#'}
#'\code{m<-ggeodata('schmatzPangea', item="tasmax_A_MO_pmip2_21k_oa_CCSM_eu_30s",startTime=1,endTime=3)}
#'\code{m<-ggeodata('schmatzPangea', item="bioclim_A_MO_pmip2_21k_oa_CCSM_eu_30s",data="bio_1")}
#'\code{TT<- ggeodata('schmaztLGMData', item='TT_Luterbacher_Xoplaki_1659-1998')}
#'}
#'
...
#'    \code{ggeodata('OSMp', extent=c(11.35547,11.40009,47.10114,47.13512), key='natural',val='peak',taglist=c('name','ele'))}\cr \cr
#'}
#'
...
#'
#'\code{ggeodata('tiroldem', item = 'KU_DGM10')} \cr
#'}
...
#' r<-ggeodata(name="SRTM",xtent = extent(11.,11.,50.,50.))
#' ## get SRTM data for an area
#' r<-ggeodata(name="SRTM",xtent = extent(11.,17.,50.,56.))
#' ## get SRTM data for an area with a buffer zone (e.g. for a cost or watershed analysis) zone is in degree
#' r<-ggeodata(name="SRTM",xtent = extent(11.,17.,50.,56.), zone = 3.0)
#' ## get SRTM Tile names
#' t<-ggeodata(name="SRTM",xtent = extent(11.,17.,50.,56.), zone = 3.0, download = FALSE)
#' ## get SRTM data for an area with a buffer and merge it
#' r<-ggeodata(name="SRTM",xtent = extent(11.,17.,50.,56.), zone = 3.0, merge = TRUE)
#'
#' ## get Schmatz et al. data please have a look at details
#' r<- ggeodata('schmatzPangea', item='tasmin_A_MO_pmip2_21k_oa_CNRM_eu_30s',endTime=12)
#' r<- ggeodata('schmatzPangea', item="bioclim_A_MO_pmip2_21k_oa_CCSM_eu_30s", layer="bio_1")
#'
#' ## get a single tile of the Tirolean DEM
#' r<- ggeodata('tiroldem', items='IBK_DGM10')
#' ## get a single 3 tiles of the Tirolean DEM as a merged raster
#' r<- ggeodata('tiroldem', item=c('IBK_DGM10','IL_DGM10','IM_DGM10'), merge =TRUE)
#'
#' # get arbitrary OSM point data
#' r<- ggeodata('OSMp', extent=c(11.35547,11.40009,47.10114,47.13512), key='natural',val='saddle',taglist=c('name','ele','direction'))
#'
#' # get Harald Breitkreutz' summit list
#' r<- ggeodata('harrylist', extent=c(11.35547,11.40009,47.10114,47.13512))
#'
#' ### the following datasets are retrieved according to Hijmans \code{getData}
#' r<- ggeodata('worldclim', var='tmin', res=0.5, lon=5, lat=45)
#' r<- ggeodata('worldclim', var='bio', res=10)
#' r<- ggeodata('CMIP5', var='tmin', res=10, rcp=85, model='AC', year=70)
#' v<- ggeodata('alt', country='FRA', mask=TRUE)
#' v<- ggeodata('GADM', country='FRA', level=1)
#' t<- ccodes()
#' }


ggeodata <- function(name='GADM', download=TRUE, path='', ...) {
  library(raster)
  library(osmar)
  library(sp)
  library(maptools)
